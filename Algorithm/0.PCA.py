...
for i in range(X_train.shape[1]):
    print("%d = %.2f" % (i + 1, float(np.sum(pca_n64.explained_variance_ratio_[:i + 1]) * 100)))

# 40维knn
pca_n40 = PCA(n_components=40)  # 降到40维
pca_n40.fit(X_train)  # 拟合X_train
X_train_pca_n40 = pca_n40.transform(X_train)  # 返回降维结果
X_test_pca_n40 = pca_n40.transform(X_test)  # 测试集也用训练集拟合的模型进行降维
...
pca_r99.fit(X_train)  # 拟合X_train
X_train_pca_r99 = pca_r99.transform(X_train)  # 返回降维结果
X_test_pca_r99 = pca_r99.transform(X_test)  # 测试集也用训练集拟合的模型进行降维
knn_clf = KNeighborsClassifier()
t = time.time()
...
print("knn_clf      time =%.2fms, score =%f" % ((time.time() - t) * 1000, knn_clf.score(X_test, y_test)))
