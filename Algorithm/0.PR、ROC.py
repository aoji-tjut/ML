...
precisons, recalls, thresholds = precision_recall_curve(y_test, decision_score)
for i in range(thresholds.shape[0]):
    if precisons[i] == recalls[i]:
...
plt.plot(thresholds, precisons[:-1], label="precision")  # last thresholds, precisons=1, recalls=0
plt.plot(thresholds, recalls[:-1], label="recall")
plt.legend()

plt.figure("PR")
plt.plot(precisons, recalls)
